...
fn grow_mixed_state_within_unused_bits() {
    ...
    // Grow without requiring an additional block. The gap between the current length and the
    // range's beginning should be set to the same value as the range. TODO since this is fully
...
fn grow_within_unused_bits_with_overlap() {
    ...
    // Grow without requiring an additional block, but leave no gap after the current len. TODO
